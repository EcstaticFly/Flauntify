...
const addProductReview = async (req, res) => {
  try {
    ...
    if(!order){
      return res.status(403).json({success:false, message: 'Purchase product to add review!' })
    }
    ...
    if(checkExisitingReview){
      return res.status(403).json({success:false, message: 'You already reviewed this product'})
    }
    ...
    const averageReview = allReviewsOfTheProduct.reduce((sum,reviewItem)=>sum + reviewItem.reviewValue,0) / totalReviewsLength;
